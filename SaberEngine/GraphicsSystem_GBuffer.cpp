...
namespace gr
{
	...
	void GBufferGraphicsSystem::Create(gr::RenderPipeline& pipeline)
	{
		// Camera:
		...
		gBufferShader->SetUniform("emissiveIntensity", &emissiveIntensity, platform::Shader::UNIFORM_TYPE::Float);
		...
		gr::TextureTargetSet& gBufferTargets = m_gBufferStage.GetStageTargetSet();
		...
		gBufferTargets.CreateColorDepthStencilTargets(Material::GBufferAlbedo, Material::GBufferDepth);
