...
namespace dx12
{
	...
	{
		SEAssert(m_numFrames >= 2 && m_numFrames <= 3, "Invalid number of frames in flight"); // This assertion might need adjustment based on how numFramesInFlight is determined for DX12
	}
	...
	void RenderManager::Render()
	{
		...
		auto EnqueueWorkRecording = 
			...
			(std::vector<WorkRange>&& workRange, bool startGPUFrameTimer, bool stopGPUFrameTimer)
			{
				...
				{
					frameTimer = re::RenderManager::Get()->GetContext()->GetGPUTimer().StartTimer(
						cmdList->GetD3DCommandList().Get(),
						k_GPUFrameTimerName);
				}
		...
		re::RenderManager::Get()->GetContext()->GetGPUTimer().EndFrame();
	}
