...
namespace gr
{
	...
	private:
		...
		std::unique_ptr<re::BatchHandle> m_worldCoordinateAxisBatch;

		bool m_showMeshCoordinateAxis = false;
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_meshCoordinateAxisBatches;

		bool m_showLightCoordinateAxis = false;
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_lightCoordinateAxisBatches;

		bool m_showSceneBoundingBox = false;
		glm::vec4 m_sceneBoundsColor = glm::vec4(1.f, 1.f, 1.f, 0.5f);
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_sceneBoundsBatches; // This is wasteful but convenient

		bool m_showAllMeshBounds = false;
		glm::vec4 m_meshBoundsColor = glm::vec4(1.f, 0.f, 0.f, 0.5f);
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_meshBoundsBatches;

		bool m_showAllMeshPrimitiveBounds = false;
		glm::vec4 m_meshPrimBoundsColor = glm::vec4(0.f, 1.f, 0.f, 0.5f);
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_meshPrimBoundsBatches;

		bool m_showAllLightBounds = false;
		glm::vec4 m_lightBoundsColor = glm::vec4(1.f, 1.f, 0.f, 0.5f);
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_lightBoundsBatches;
		
		bool m_showAllVertexNormals = false;
		float m_vertexNormalsScale = 0.2f;
		glm::vec4 m_normalsColor = glm::vec4(0.f, 0.f, 1.f, 0.5f);
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_vertexNormalBatches;
		...
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_cameraAxisBatches;
		std::unordered_map<gr::RenderDataID, std::vector<std::unique_ptr<re::BatchHandle>>> m_cameraFrustumBatches;
		...
		std::unordered_map<gr::RenderDataID, std::unique_ptr<re::BatchHandle>> m_deferredLightWireframeBatches;

		bool m_showAllTransforms = false;
		std::unordered_map<gr::TransformID, std::unique_ptr<re::BatchHandle>> m_transformAxisBatches;

		bool m_showParentChildLinks = false;
		std::unordered_map<gr::TransformID, std::unique_ptr<re::BatchHandle>> m_transformParentChildLinkBatches;
