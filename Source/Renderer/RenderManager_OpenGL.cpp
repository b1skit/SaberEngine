...
namespace opengl
{
	...
	void RenderManager::PlatformCreateAPIResources()
	{
		...
		// Textures:
		if (this->m_newTextures.HasReadData())
		{
			SEBeginCPUEvent("Create textures");
			for (auto const& newObject : this->m_newTextures.GetReadData())
			{
		...
		// Samplers:
		if (this->m_newSamplers.HasReadData())
		{
			SEBeginCPUEvent("Create samplers");
			for (auto& newObject : this->m_newSamplers.GetReadData())
			{
		...
		if (this->m_newShaders.HasReadData())
		{
			...
			for (auto& newObject : this->m_newShaders.GetReadData())
			{
