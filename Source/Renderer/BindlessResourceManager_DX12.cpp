...
namespace
{
	...
	std::unique_ptr<dx12::RootSignature> CreateGlobalBRMRootSignature(dx12::Context* context)
	{
		...
		tableRanges.emplace_back(dx12::RootSignature::DescriptorRangeCreateDesc{
			.m_shaderName = RTAOParamsData::s_shaderName,
			.m_rangeDesc = D3D12_DESCRIPTOR_RANGE1{
				.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV,
				.NumDescriptors = std::numeric_limits<uint32_t>::max(), // Unbounded
				.BaseShaderRegister = 0,
		...
		tableRanges.emplace_back(dx12::RootSignature::DescriptorRangeCreateDesc{
			.m_shaderName = "Texture2DRWFloat2",
			.m_rangeDesc = D3D12_DESCRIPTOR_RANGE1{
				.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV,
				.NumDescriptors = std::numeric_limits<uint32_t>::max(), // Unbounded
				.BaseShaderRegister = 0,
		...
		tableRanges.emplace_back(dx12::RootSignature::DescriptorRangeCreateDesc{
			.m_shaderName = "Texture2DRWFloat3",
			.m_rangeDesc = D3D12_DESCRIPTOR_RANGE1{
				.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV,
				.NumDescriptors = std::numeric_limits<uint32_t>::max(), // Unbounded
				.BaseShaderRegister = 0,
		...
		re::BindlessResourceManager& brm, uint8_t numFramesInFlight, uint64_t frameNum)
	{
			...
			brm.GetPlatformObject()->As<dx12::BindlessResourceManager::PlatObj*>();
		{
			...
			if (brmPlatObj->m_isCreated == false) // First initialization: 
			{
				...
				brmPlatObj->m_nullDescriptor = ctx->GetNullSRVDescriptor(
