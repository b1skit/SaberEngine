...
namespace platform
{
	...
	std::unique_ptr<re::BindlessResourceManager::PlatformParams>
		BindlessResourceManager::CreatePlatformParams(re::BindlessResourceManager& bindlessResourceMgr)
	{
		const platform::RenderingAPI api = re::RenderManager::Get()->GetRenderingAPI();
	...
	void (*BindlessResourceManager::Create)(re::BindlessResourceManager&, uint32_t totalDescriptors) = nullptr;
}