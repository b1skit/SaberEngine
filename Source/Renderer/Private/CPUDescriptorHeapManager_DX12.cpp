...
namespace dx12
{
	...
	CPUDescriptorHeapManager::CPUDescriptorHeapManager(HeapType type)
		...
		, m_elementSize(re::RenderManager::Get()->GetContext()->As<dx12::Context*>()->GetDevice().GetD3DDevice()
		...
		, m_numFreeElements(0) // Updated when we make our 1st FreeRange() call
	{
		...
		HRESULT hr = re::RenderManager::Get()->GetContext()->As<dx12::Context*>()->GetDevice().GetD3DDevice()->CreateDescriptorHeap(
			&heapDescriptor, 
