...
//	vec3 fresnel_kS = FresnelSchlick(NoV, F0); // Doesn't quite look right: Use FresnelSchlick_Roughness() instead
	...
	const vec2 BRDF = texture(Tex7, vec2(max(NoV, 0.0), gbuffer.Roughness) ).rg; // Sample our generated BRDF Integration map

	const vec3 specular = textureLod(CubeMap1, worldReflection, gbuffer.Roughness * g_maxPMREMMip).xyz * ((fresnel_kS * BRDF.x) + BRDF.y);
