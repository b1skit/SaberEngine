...
vec3 GetDiffuseIBLContribution(vec3 N, vec3 V, float NoV, float remappedRoughness)
{
	...
	const vec3 diffuseLighting = texture(CubeMapIEM, WorldToCubeSampleDir(dominantN)).rgb; // IEM

	const float fDiffuse = texture(DFG, vec2(NoV, remappedRoughness), 0).z;
	...
	vec3 N, vec3 R, vec3 V, float NoV, float linearRoughness, float remappedRoughness, vec3 blendedF0)
{
	...
	const vec3 preIntegratedLD = textureLod(CubeMapPMREM, WorldToCubeSampleDir(dominantR), mipSampleLevel).rgb;

	// Sample the pre-integrated DFG texture
	//	Fc = (1.f - LoH)^5
	//	PreIntegratedDFG.r = GVis * (1.f - Fc)
	//	PreIntegratedDFG.g = GVis * Fc	
	const vec2 preIntegratedDFG = texture(DFG, vec2(NoV, remappedRoughness), 0).xy;
...
vec3 ComputeAmbientLighting(AmbientLightingParams lightingParams)
{	
	const vec3 blendedF0 = ComputeBlendedF0(
	...
	const float NoV = clamp(dot(lightingParams.WorldNormal, lightingParams.V), 0.f, 1.f);

	const vec3 diffuseIlluminance = GetDiffuseIBLContribution(
		lightingParams.WorldNormal,
		lightingParams.V, 
		NoV, 
		lightingParams.RemappedRoughness);

	const vec3 R = reflect(-lightingParams.V, lightingParams.WorldNormal);

	const vec3 specularIlluminance = GetSpecularIBLContribution(
		lightingParams.WorldNormal,
		R,
		lightingParams.V,
		NoV,
