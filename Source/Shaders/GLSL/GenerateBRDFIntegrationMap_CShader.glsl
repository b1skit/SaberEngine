...
layout(location=0) writeonly uniform image2D output0;
...
void CShader()
{
	...
	vec2 screenUV = PixelCoordsToScreenUV(
		...
		_BRDFIntegrationParams.g_integrationTargetResolution.xy, 
		vec2(0.5f, 0.5f), 
		false);
	...
	imageStore(output0, texelCoord, result / NUM_SAMPLES);
}