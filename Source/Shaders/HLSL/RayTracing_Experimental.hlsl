...
// This shader is currently just a copy/paste from the DXR raytracing tutorial, so we have something valid to compile
...
[shader("closesthit")]
void ClosestHit_Experimental(inout HitInfo payload, Attributes attrib)
{
	float3 barycentrics =
...
[shader("anyhit")]
void AnyHit_Experimental(inout HitInfo payload, Attributes attrib)
{
	float3 barycentrics =
	  float3(1.f - attrib.bary.x - attrib.bary.y, attrib.bary.x, attrib.bary.y);
...
[shader("miss")]
void Miss_Experimental(inout HitInfo payload : SV_RayPayload)
{
	uint2 launchIndex = DispatchRaysIndex().xy;
	float2 dims = float2(DispatchRaysDimensions().xy);
...
[shader("callable")]
void Callable_Experimental(inout HitInfo payload : SV_RayPayload)
{
	//
}