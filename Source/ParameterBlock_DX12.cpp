...
namespace dx12
{
	void ParameterBlock::Create(re::ParameterBlock& paramBlock)
	{
		...
		HRESULT hr = device->CreateCommittedResource(
			...
			IID_PPV_ARGS(&params->m_constantBufferResource));
		CheckHResult(hr, "Failed to create committed resource");

		params->m_constantBufferResource->SetName(paramBlock.GetWName().c_str());
		...
		constantBufferViewDesc.BufferLocation = params->m_constantBufferResource->GetGPUVirtualAddress();
	...
	void ParameterBlock::Update(re::ParameterBlock& paramBlock)
	{
		...
		HRESULT hr = params->m_constantBufferResource->Map(
			0,						// Subresource
			&readRange,
			&cpuVisibleData);
		...
		params->m_constantBufferResource->Unmap(
			0,
	...
	void ParameterBlock::Destroy(re::ParameterBlock& paramBlock)
	{
		...
		params->m_constantBufferResource = nullptr;		
